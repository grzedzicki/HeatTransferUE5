...
FThreadCalculations::FThreadCalculations(int32 _grid_size_1, int32 _grid_size_2, int32 _grid_size_3, float _alpha, float _delta_x, float _delta_y, float _delta_z, float _delta_t, TArray<float> _CalculationArray, TArray<bool> _BoolArray, AThreadActor* FunActor)
{
	if (FunActor) {
...
{
	UE_LOG(LogTemp, Warning, TEXT("FThreadCalculations::Run"));
	CalculateArray();
